...
name_simulation = "likelihood_ju_SIS_"+str(N)+"_V2"

input_path  = "CAL/Data/Likelihood/Input/ju_SIS/"
output_path = "CAL/Data/Likelihood/Output/ju_SIS_V2/"+str(N)+"/"
...
Y_SIS_N = tf.convert_to_tensor(np.load(input_path+"ju_Y_"+str(N)+".npy"), dtype=tf.float32)[:N,:]

ju_covariates = tf.concat((tf.ones(tf.shape(covariates)), covariates), axis = 1)
SIS_N    = simba_SIS(ju_covariates)
