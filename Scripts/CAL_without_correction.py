...
def CAL_without_correction_without_extra(ibm, parameters, y):
	...
	def body(input, t):
		...
		pi_tm1, nu_tm1, _, _ = input

		k_nu_tm1 = ibm.K_x(parameters, nu_tm1)

		nu_t     = CAL_prediction(nu_tm1, k_nu_tm1)
		pi_t_tm1 = CAL_prediction(pi_tm1, k_nu_tm1)

		g_t = ibm.G_t(parameters)
		pi_t, mu_t, log_likelihood_increment = CAL_update(pi_t_tm1, g_t, y[t,...])

		return pi_t, nu_t, mu_t, log_likelihood_increment

	Pi, Nu, Mu, Log_likelihood = tf.scan(body, tf.range(0, T), initializer = (pi_0, nu_0, mu_0, log_likelihood))

	return tf.concat((tf.expand_dims(pi_0, axis = 0), Pi), axis = 0), tf.concat((tf.expand_dims(nu_0, axis = 0), Nu), axis = 0), Mu, tf.reduce_sum(tf.reduce_sum(Log_likelihood, axis = 1)) #tf.reduce_sum(tf.math.log(tf.reduce_mean(tf.math.exp(Log_likelihood), axis = 1)))

def CAL_without_correction_with_extra(ibm, parameters, y):
	...
	def body(input, t):
		...
		pi_tm1, nu_tm1, _, _, extra_state = input

		k_nu_tm1 = ibm.K_x(parameters, nu_tm1, extra_state)
		...
		return pi_t, nu_t, mu_t, log_likelihood_increment, extra_state

	Pi, Nu, Mu, Log_likelihood, _ = tf.scan(body, tf.range(0, T), initializer = (pi_0, mu_0, log_likelihood, extra_state))

	return tf.concat((tf.expand_dims(pi_0, axis = 0), Pi), axis = 0), tf.concat((tf.expand_dims(nu_0, axis = 0), Nu), axis = 0), Mu, tf.reduce_sum(tf.reduce_sum(Log_likelihood, axis = 1)) 

def CAL_without_correction(ibm, parameters, y):

	if hasattr(ibm, 'extra') and callable(getattr(ibm, 'extra')):
		return CAL_without_correction_with_extra(ibm, parameters, y)

	else:
		return CAL_without_correction_without_extra(ibm, parameters, y)

@tf.function(jit_compile=True)
def CAL_without_correction_compiled(ibm, parameters, y):
...
def CAL_without_correction_loss(ibm, parameters, y):

    _, _, _, log_likelihood = CAL_without_correction(ibm, parameters, y)

    return -log_likelihood/tf.cast(ibm.N, dtype = tf.float32)

@tf.function(jit_compile=True)
def CAL_without_correction_grad_loss(ibm, parameters, y, learning_parameters):

    with tf.GradientTape() as g:
        # for key in learning_parameters:
        #     g.watch(parameters[key])

        loss = CAL_without_correction_loss(ibm, parameters, y)

    return loss, g.gradient(loss, [parameters[key] for key in learning_parameters])

def CAL_without_correction_inference(ibm, parameters, y, learning_parameters, optimizer, n_gradient_steps, seed_optim = None, initialization = "random"):
	...
	if initialization == "random":
		...
		current_loss = CAL_without_correction_loss(ibm, parameters, y)
	...
	for i in range(n_gradient_steps):
		loss, grad = CAL_without_correction_grad_loss(ibm, parameters, y, learning_parameters)
